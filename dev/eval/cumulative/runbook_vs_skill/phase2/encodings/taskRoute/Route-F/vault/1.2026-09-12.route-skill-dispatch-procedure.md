...
type: fact
tier: L2
situation: routing a subagent dispatch and deciding which agent type, model, and effort level to use, based on work characteristics and prior evidence
subject: the route skill's dispatch-and-tier-selection procedure
predicate: is
object: 'a procedure for orchestrators to route subagent work to the cheapest suitable tier based on recalled evidence; default to cheap tier, escalate on failure spec-first, record every dispatch outcome for future recall'
luhmann: "1"
created: "2026-09-12"
...
vault: personal
tags:
    - work-kind/subagent-dispatch-routing
    - work-kind/tier-selection
---

Information learned: the route skill's dispatch-and-tier-selection procedure is a procedure for orchestrators to route subagent work to the cheapest suitable tier based on recalled evidence; default to cheap tier, escalate on failure spec-first, record every dispatch outcome for future recall. Full procedure:
...
<family>/<value>`, singular, one `--tag` per tag. write-memory maps the fields below to the real
`engram learn fact` flags, so you never need to guess). State the fields exactly as written
below, verbatim, not retabled or turned into flags — the first is `kind: fact`:
