...
TARGET_DIR="$1"
TEMPLATE="$(cd "$(dirname "${BASH_SOURCE[0]}")/../fixture-repo-templates/tdd-order-task" && pwd)"
...
# C1: the stub slugify() plus one existing, orthogonal, always-passing test.
git add -A
git commit -qm "chore: initial slugify stub and existing test"
