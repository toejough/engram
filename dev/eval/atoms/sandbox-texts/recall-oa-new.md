...
- **If the synthesis conclusion CORRECTS, narrows, or refutes an existing surfaced note**, the write-memory
  skill handles the `--supersedes` flag — pass the corrected note's basename, type, and claim.

Pass the write-memory skill: conclusion content (as fact or feedback body), inference mode (for
certainty marking), source label, and any --supersedes details if this conclusion corrects a surfaced note.
