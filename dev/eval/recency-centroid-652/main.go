// Command recency-centroid-652 is a throwaway eval harness for issue #652:
...
//
// Not part of the production build — run via `go run ./dev/eval/recency-centroid-652`
// and delete/keep as a reference; it imports internal/cluster and internal/embed
// directly since it lives in the same module.
package main
...
var supersedesLinkRE = regexp.MustCompile(`\[\[([^\]|]+)`)

func main() {
	notes, err := loadVault(vaultDir)
	if err != nil {
		fmt.Fprintln(os.Stderr, "load vault:", err)
		os.Exit(1)
	}
	...
	for _, n := range notes {
		for _, target := range n.supersedes {
			...
			pr.unweightedOldRank, pr.unweightedOldInTop = rankOf(uRanked, old.path)
			pr.unweightedNewRank, pr.unweightedNewInTop = rankOf(uRanked, n.path)
			pr.weightedOldRank, pr.weightedOldInTop = rankOf(wRanked, old.path)
			pr.weightedNewRank, pr.weightedNewInTop = rankOf(wRanked, n.path)

			pr.unweightedRelevance = meanQueryCosine(uRanked[:min(candidateNoteK, len(uRanked))], queryVec)
			pr.weightedRelevance = meanQueryCosine(wRanked[:min(candidateNoteK, len(wRanked))], queryVec)
	...
	fmt.Printf("supersedes: pairs found=%d, tested=%d, no-old-target=%d, cluster<=5=%d, eligible(same-cluster,>5)=%d\n",
		tested+skippedNoOld, tested, skippedNoOld, skippedSmallCluster, len(results))
	fmt.Println()

	if len(results) == 0 {
		fmt.Println("NO eligible test cases (same-cluster supersession pairs in a >5-member cluster). Cannot run the eval on real data.")
		return
	}
