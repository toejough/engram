...
func TestDiff(t *testing.T) {
	...
	t.Run("writes heatmap as jpeg", func(t *testing.T) {
		g := NewWithT(t)
		fs := &MockFS{Files: make(map[string][]byte)}
		dir := t.TempDir()
		...
		path1 := savePNG(t, fs, dir, "expected.png", img)
		path2 := savePNG(t, fs, dir, "actual.png", img)
		heatmapPath := filepath.Join(dir, "heatmap.jpg")
	...
	t.Run("errors when heatmap create fails", func(t *testing.T) {
		g := NewWithT(t)
		baseFS := &MockFS{Files: make(map[string][]byte)}
		dir := t.TempDir()

		img := solidImage(50, 50, color.RGBA{R: 128, G: 128, B: 128, A: 255})
		path1 := savePNG(t, baseFS, dir, "expected.png", img)
		path2 := savePNG(t, baseFS, dir, "actual.png", img)

		errFS := &failCreateFS{MockFS: baseFS}
		_, err := screenshot.Diff(path1, path2, screenshot.DiffOpts{
			HeatmapOutput: filepath.Join(dir, "heatmap.png"),
		}, errFS)
		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Error()).To(ContainSubstring("failed to save heatmap"))
	})
}
...
// failCreateFS wraps MockFS but returns an error from Create.
type failCreateFS struct {
	*MockFS
}

func (f *failCreateFS) Create(_ string) (io.WriteCloser, error) {
