...
func TestSelectQuestions_ReferencesGatheredContext(t *testing.T) {
	...
	g.Expect(questions).ToNot(BeEmpty())
...
// TestSelectQuestions_SmallGap_ManyUnanswered verifies small gap caps at 2 questions
// even when more than 2 questions are unanswered (exercises return smallGapMaxQuestions path).
func TestSelectQuestions_SmallGap_ManyUnanswered(t *testing.T) {
	t.Parallel()
	g := NewWithT(t)

	keyQuestions := []interview.KeyQuestion{
		{ID: "q1", Text: "Q1?", Priority: interview.PriorityCritical},
		{ID: "q2", Text: "Q2?", Priority: interview.PriorityCritical},
		{ID: "q3", Text: "Q3?", Priority: interview.PriorityCritical},
	}

	gapAnalysis := interview.GapAnalysis{
		CoveragePercent:     85.0,
		GapSize:             interview.GapSizeSmall,
		UnansweredQuestions: []string{"q1", "q2", "q3"},
	}

	gathered := map[string]string{}
	questions := interview.SelectQuestions(keyQuestions, gapAnalysis, gathered)
	g.Expect(questions).To(HaveLen(2))
}
...
// TestSelectQuestions_UnknownGapSize verifies that an unknown GapSize returns 0 questions
// (exercises the default branch of determineMaxQuestions).
func TestSelectQuestions_UnknownGapSize(t *testing.T) {
	t.Parallel()
	g := NewWithT(t)
	...
	gapAnalysis := interview.GapAnalysis{
		CoveragePercent:     60.0,
		GapSize:             interview.GapSize("unknown"),
		UnansweredQuestions: []string{"q1"},
	}
