...
func TestGraph_AddEdge(t *testing.T) {
	...
	if len(graph.Edges["REQ-1"]) < 1 {
		t.Fatal("expected at least 1 forward edge")
	}
	...
	if len(graph.ReverseEdges["ARCH-1"]) < 1 {
		t.Fatal("expected at least 1 reverse edge")
	}
...
func TestGraph_AddNode_StateUnchangedOnError(t *testing.T) {
	...
	if graph.Nodes["REQ-1"] == nil {
		t.Fatal("expected REQ-1 node to be non-nil")
	}
...
func TestGraph_Orphans_PropertyKnownGaps(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		orphans := graph.Orphans(trace.NodeTypeREQ, "downstream")
		g.Expect(orphans).To(HaveLen(orphanCount))
	})
}
...
func TestGraph_Upstream_PropertyRandomDAG(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		if chainLen > 0 {
			...
			g.Expect(err).ToNot(HaveOccurred())
			g.Expect(ancestors).To(HaveLen(chainLen - 1))
		}
	})
}
