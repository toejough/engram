...
func (m *Manager) DetectBaseBranch() (string, error) {
	...
	if branch == "" || branch == "HEAD" {
		return "", errors.New("failed to detect base branch: detached HEAD")
	}
	...
	for _, line := range lines {
		if after, ok := strings.CutPrefix(line, "worktree "); ok {
			...
			if strings.HasPrefix(currentPath, parentDir) {
				if after, ok := strings.CutPrefix(currentBranch, "task/"); ok {
					taskID := after
				...
				} else if after, ok := strings.CutPrefix(currentBranch, "project/"); ok {
					projectName := after
					worktrees = append(worktrees, WorktreeInfo{
						TaskID: projectName,
						Path:   currentPath,
