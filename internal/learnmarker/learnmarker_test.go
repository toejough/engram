...
func TestOSFS_MkdirAll_WrapsError(t *testing.T) {
	t.Parallel()
	g := gomega.NewWithT(t)

	// Try to create a dir under a file path — guaranteed to fail.
	tmp := t.TempDir()
	filePath := tmp + "/notadir"
	g.Expect(os.WriteFile(filePath, []byte("x"), 0o600)).To(gomega.Succeed())

	err := learnmarker.OSFS{}.MkdirAll(filePath+"/child", 0o755)

	g.Expect(err).To(gomega.MatchError(gomega.ContainSubstring("OSFS.MkdirAll")))
}

func TestOSFS_ReadFile_WrapsError(t *testing.T) {
	t.Parallel()
	g := gomega.NewWithT(t)

	_, err := learnmarker.OSFS{}.ReadFile("/nonexistent/path/that/cannot/exist/abc123")

	g.Expect(err).To(gomega.MatchError(gomega.ContainSubstring("OSFS.ReadFile")))
}

func TestOSFS_WriteFile_WrapsError(t *testing.T) {
	t.Parallel()
	g := gomega.NewWithT(t)

	err := learnmarker.OSFS{}.WriteFile("/nonexistent/path/abc123/file.txt", []byte("x"), 0o644)

	g.Expect(err).To(gomega.MatchError(gomega.ContainSubstring("OSFS.WriteFile")))
}
...
func TestWrite_PropagatesMkdirError(t *testing.T) {
	t.Parallel()
	g := gomega.NewWithT(t)
	fs := &fakeFS{files: map[string][]byte{}, mkdirErr: errors.New("permission denied")}
