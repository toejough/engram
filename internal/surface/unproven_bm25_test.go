...
// TestUnprovenToolMemoryFilteredByHigherBM25Floor verifies that an unproven anti-pattern
// memory with weak BM25 relevance is filtered out in tool mode (floor=0.30), while a
// proven memory with the same score passes.
func TestUnprovenToolMemoryFilteredByHigherBM25Floor(t *testing.T) {
	t.Parallel()

	g := NewGomegaWithT(t)

	unprovenPath := "unproven-commit.toml"
	provenPath := "proven-commit.toml"

	// 21 docs total: 2 targets + 8 fillers with "commit" + 11 fillers without.
	// "commit" appears in 10/21 docs → IDF ≈ 0.091.
	// Unproven gets 0.30 floor → 0.091 < 0.30 → filtered.
	// Proven gets 0.05 floor → 0.091 > 0.05 → passes.
	memories := make([]*memory.Stored, 0, 21)
	memories = append(memories,
		&memory.Stored{
			...
			Principle:   "review before commit",
		},
		&memory.Stored{
			Title:       "Proven Rule",
			FilePath:    provenPath,
	...
	for i := range 8 {
		memories = append(memories, &memory.Stored{
			Title:       fmt.Sprintf("Filler Commit %d", i),
			FilePath:    fmt.Sprintf("filler-commit-%d.toml", i),
			AntiPattern: fmt.Sprintf("commit filler %d", i),
			Keywords:    []string{"commit"},
			Principle:   fmt.Sprintf("commit standard %d", i),
		})
	}
	...
	for i := range 11 {
		memories = append(memories, &memory.Stored{
			Title:       fmt.Sprintf("Filler Other %d", i),
			FilePath:    fmt.Sprintf("filler-other-%d.toml", i),
			AntiPattern: fmt.Sprintf("other filler %d", i),
			Keywords:    []string{"other"},
			Principle:   fmt.Sprintf("other standard %d", i),
		})
	}

	stats := map[string]surface.EffectivenessStat{
		provenPath: {SurfacedCount: 1, EffectivenessScore: 50.0},
		// unprovenPath absent → isUnproven returns true → floor = 0.30.
	}
	...
	output := buf.String()
	g.Expect(output).NotTo(
		ContainSubstring("unproven-commit"),
		"unproven memory should be filtered by higher tool BM25 floor",
	)
	g.Expect(output).To(
		ContainSubstring("proven-commit"),
		"proven memory should pass the lower tool BM25 floor",
	)
}
