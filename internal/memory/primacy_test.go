...
func TestFormatMarkdownPrimacyOrdering(t *testing.T) {
	...
	err := memory.Learn(memory.LearnOpts{
		Message:    "never use git checkout dash dot",
		Type:       "correction",
	...
	if queryResults == nil {
		t.Fatal("Query returned nil queryResults")
	}
...
func TestSortByPrimacyAllCorrections(t *testing.T) {
	...
	if len(sorted) < 3 {
		t.Fatalf("expected 3 results from SortByPrimacy, got %d", len(sorted))
	}
...
func TestSortByPrimacyPropertyStableOrdering(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		g.Expect(sorted1).To(HaveLen(len(sorted2)))
