...
func TestGetFeedbackStats(t *testing.T) {
	...
	if stats == nil {
		t.Fatal("GetFeedbackStats returned nil stats")
	}
...
func TestPropagateEmbeddingFeedbackToSkills_FlaggedSource(t *testing.T) {
	...
	if len(result.Results) < 2 {
		t.Fatalf("expected at least 2 results from Query, got %d", len(result.Results))
	}
...
func TestRecordFeedback_Helpful(t *testing.T) {
	...
	if result2 == nil {
		t.Fatal("Query returned nil result2")
	}

	g.Expect(result2.Results).ToNot(BeEmpty())
...
func TestRecordFeedback_Unclear(t *testing.T) {
	...
	if len(flagged) < 1 {
		t.Fatal("expected at least 1 flagged entry from ListFlaggedForRewrite")
	}
...
func TestRecordFeedback_Wrong(t *testing.T) {
	...
	if len(result2.Results) < 1 {
		t.Fatal("expected at least 1 result from Query (result2)")
	}
	...
	if len(flagged) < 1 {
		t.Fatal("expected at least 1 flagged entry from ListFlaggedForReview")
	}
