...
func applySplit(db *sql.DB, proposal MaintenanceProposal) error {
	...
	return errors.New("split action not yet implemented")
}
...
func extractRationaleFromEnriched(enriched string) string {
	// Try to split on " - "
	if _, after, ok := strings.Cut(enriched, " - "); ok {
		return strings.TrimSpace(after)
	}

	// Try to split on " because "
	if _, after, ok := strings.Cut(enriched, " because "); ok {
		return strings.TrimSpace(after)
	}
...
func scanPromotableEmbeddings(db *sql.DB) ([]MaintenanceProposal, error) {
	...
	for rows.Next() {
		var (
			id                              int64
			content, projectsStr, principle string
			confidence                      float64
			retrievalCount                  int
		)

		err := rows.Scan(&id, &content, &confidence, &retrievalCount, &projectsStr, &principle)
		if err != nil {
			continue
		}
