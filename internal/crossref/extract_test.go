...
`
		ext := crossref.ClaudeMDExtractor{
			Content:    content,
	...
	t.Run("empty content produces zero entries", func(t *testing.T) {
		...
		ext := crossref.ClaudeMDExtractor{Content: "", SourcePath: "CLAUDE.md"}
	...
	t.Run("stable IDs across extractions", func(t *testing.T) {
		...
		ext := crossref.ClaudeMDExtractor{Content: content, SourcePath: "CLAUDE.md"}
	...
	t.Run("skips bullet lines with empty text after prefix", func(t *testing.T) {
		t.Parallel()
		g := NewWithT(t)
		...
		g.Expect(entries).To(HaveLen(1))
		g.Expect(entries[0].Title).To(Equal("Real entry"))
	})
}
...
- Never auto-commit
`
		ext := crossref.MemoryMDExtractor{
			Content:    content,
	...
	t.Run("empty content produces zero entries", func(t *testing.T) {
		...
		ext := crossref.MemoryMDExtractor{Content: "", SourcePath: "MEMORY.md"}
...
func TestRuleExtractor_Extract(t *testing.T) {
	...
	t.Run("empty content produces zero entries", func(t *testing.T) {
		...
		ext := crossref.RuleExtractor{Filename: "go.md", Content: ""}
...
func TestSkillExtractor_Extract(t *testing.T) {
	...
	t.Run("empty content produces zero entries", func(t *testing.T) {
		...
		ext := crossref.SkillExtractor{SkillName: "commit", Content: ""}
