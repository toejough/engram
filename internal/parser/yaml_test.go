...
func TestParseDocument_PropertyValidItems(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		var contentSb146 strings.Builder

		for i := range itemCount {
			id := "REQ-" + paddedNum(i+1)

			contentSb146.WriteString("---\n")
			contentSb146.WriteString("id: " + id + "\n")
			contentSb146.WriteString("type: REQ\n")
			contentSb146.WriteString("project: test-project\n")
			contentSb146.WriteString("title: Requirement " + paddedNum(i+1) + "\n")
			contentSb146.WriteString("status: active\n")
			contentSb146.WriteString("created: 2024-01-15T10:30:00Z\n")
			contentSb146.WriteString("updated: 2024-01-16T14:00:00Z\n")
			contentSb146.WriteString("---\n\n")
			contentSb146.WriteString("Body " + paddedNum(i+1) + ".\n\n")
		}

		content += contentSb146.String()
...
func TestSplitFrontmatter_PropertyValidItems(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		for i := range itemCount {
			...
			contentSb582.WriteString("---\n")
			contentSb582.WriteString("id: " + id + "\n")
			contentSb582.WriteString("title: " + title + "\n")
			contentSb582.WriteString("---\n")
			contentSb582.WriteString("\n")
			...
			if rapid.Bool().Draw(rt, "hasBody") {
				...
				contentSb582.WriteString(body + "\n")
			}

			contentSb582.WriteString("\n")
		}

		content += contentSb582.String()

		items, err := parser.SplitFrontmatter(content)
		g.Expect(err).ToNot(HaveOccurred())
		g.Expect(items).To(HaveLen(itemCount))
