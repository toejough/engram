...
| Skill | What it does |
|-------|--------------|
| `recall` | Surfaces relevant notes and raw chunks via a single `engram query --synthesize-l2` call (semantic search + 3-hop subgraph + a unified chunk+note clustering that nominates top-K `candidate_l2s` per cluster). For each cluster it judges coverage inline (covered/near/absent) and crystallizes via `engram amend` (update an existing L2) or `engram learn` (create one), then reports whether the surfaced memory changed the agent's plan. |
...
engram show <ref> [--ref <ref>...]   Print a note (frontmatter + body + outbound wikilink targets), read-only. Used by /recall to read candidate L2s before judging coverage.
