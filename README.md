...
**If your vault predates the vocab→tags migration (2026-07-10, #678), its old vocab notes are silently unrecognized** — no read path reads them, and no error is raised. Vocab membership now lives in a single `tags: [vocab/<term>]` frontmatter entry; the old `vocab.<term>.md` term-note files, `Vocab:` body-lines, `vocab:` frontmatter, and `vocab.index.md` index are retired. `engram update` prints a one-line notice pointing here when it detects old-format files in your vault. Run **`engram update --regen-vocab`** to automate the migration (preview first with `--dry-run`).

**If your chunk index predates the ingest-time dedup (2026-07-26), it may have a backlog of duplicate index files** — before this change, the same content reachable from more than one path (e.g. a project file and a copy of it swept in from an ancestor `.claude` directory) got its own `.jsonl` index file and manifest entry for every copy, which both wastes disk/scan time and lets the same content show up more than once in search results. Ingest now dedups by content hash going forward — one canonical copy is indexed per group of byte-identical sources, and a later, higher-precedence copy evicts an already-indexed lower-precedence one — but it does not retroactively clean up copies indexed before this change. Run **`engram prune --duplicates`** once to clear the backlog (preview first with `engram prune --duplicates --dry-run`) — it's safe by construction: a duplicate is only removed once its retained twin's index file is confirmed present, and any group where that can't be confirmed is refused rather than removed. `engram update` prints a one-line notice pointing here when it detects a live duplicate group in the manifest.
