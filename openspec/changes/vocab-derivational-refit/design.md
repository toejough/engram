...
4. **`proposed` provenance shield.** `vocab.centroids.json` entries gain `origin: derived|proposed`. `vocab propose`-minted terms are never retired by derivation (they may be geometrically invisible by design); they participate in assignment normally. Their existence slightly perturbs top-3 assignment but not derivation input.
...
6. **Learn skill Step 1.5 shrinks** to: `engram vocab stats` → if REFIT_PENDING, run `engram vocab refit`; the binary emits any needed naming requests (new clusters) as structured output the agent answers in one round; report loudly. Skill edit follows `superpowers:writing-skills` TDD.
...
Grep over `refit|emit-request|two-pass|refit_pending|new_terms|untagged` across docs/, agent-instructions/, openspec/specs/, CLAUDE.md:
