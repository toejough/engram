...
- [ ] 2.1 Create `internal/serve/` with route registration for exactly the served set: `query`, `query-chunks`, `show`, `show-chunk`, `activate`, `learn`, `amend` — no route for `ingest`/`vocab refit`/`prune`/`check`/`update`/`resituate`
...
## 7. `ENGRAM_SERVER` CLI-client mode

- [ ] 7.1 When `ENGRAM_SERVER` is set, route the served command set's CLI targets (`query`, `query-chunks`, `show`, `show-chunk`, `activate`, `learn`, `amend`) through an HTTP client instead of local file I/O, with identical command syntax and output shape
- [ ] 7.2 Confirm host-only commands (`ingest`, `vocab refit`, `prune`, `check`, `update`, `resituate`) are unaffected by `ENGRAM_SERVER` — they always run against local files

## 8. Curation skill (follow-on, outside this change's Go-code scope)

- [ ] 8.1 Author a curation skill under the `superpowers:writing-skills` convention (per CLAUDE.md, mandatory for any SKILL.md edit) that judges pending offers against the host vault using the same covered/near/absent reasoning `recall`'s Step 2.5 already documents, issuing `engram amend` (near) / `engram learn` (absent) / discard (covered), clearing the pending-offer marker in each case
- [ ] 8.2 Confirm the skill never runs synchronously inside a served HTTP request — it is invoked separately (see design.md Open Questions for cadence)

## 9. Tests

- [ ] 9.1 Served command set: each of the seven served commands reachable and produces the same result as local invocation; each host-only command has no route
- [ ] 9.2 Bind address: starting `engram serve` without an explicit address refuses to start
- [ ] 9.3 Concurrency: a local writer and a served writer racing the same vault — no lost update (reuses the existing concurrent-writers regression pattern, ADR-0013)
- [ ] 9.4 Parity: a fixed `query` invocation returns byte-identical output via local CLI vs. `ENGRAM_SERVER`-routed CLI
- [ ] 9.5 Identity: a served write with a spoofed client-supplied `user:` still lands with the Cloudflare-authenticated identity; `repo:`/`vault:` are unaffected by the server
- [ ] 9.6 Offer marker: a served `learn`/`amend` produces a note carrying the pending-offer marker; `query` excludes it; the marker is absent from notes created by local (non-served) `learn`/`amend`
- [ ] 9.7 Pending-offer detection: fires immediately on a single pending offer (no batching) at all three surfacing points (query payload, update notice, write-path log); silent when none exist
- [ ] 9.8 Fire-and-forget: the served API's response to a write that creates a pending offer contains no curation-outcome field

## 10. Verification

- [ ] 10.1 `targ check-full` green
- [ ] 10.2 Real-binary check: run `engram serve` against a scratch vault, drive each served command from a second `engram` invocation with `ENGRAM_SERVER` set, confirm parity and locking against a concurrent local writer
- [ ] 10.3 Real-binary check: a served `learn` with a Cloudflare Access test header produces a pending-offer note stamped with that identity; `engram query`/`engram update` surface its existence; a local `engram learn` in the same vault does not trigger the pending-offer marker or its notices
