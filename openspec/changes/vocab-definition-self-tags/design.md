...
- **D2 — semantics preserved by exclusion at read sites.** Tag nomination gains an explicit definition filter (`query_nominations.go`); stats/trigger member and untagged math are verified to already exclude definitions via the marker and pinned by tests either way. Alternative (let definitions become members): rejected as a behavior change outside the ask.
- **D3 — backfill is a new explicit idempotent subcommand, `engram vocab tag-definitions`.** Walks definition notes, derives each term from the `vocab-<term>-definition` slug (constants at `vocab_commands.go:313-316`), adds the missing self-tag, reports per-note added/already-present. Writes go through the existing locked vault write path so each touched note's sidecar content hash is refreshed (tag edits change file bytes; body/situation text is unchanged, so re-embedding is cheap — ~29 notes, in-binary model, seconds). Alternative (fold into `engram update` or refit): rejected — update must not mutate (prior incident, vault note 532), and a refit is a heavyweight unrelated operation.
- **D4 — minting writes the self-tag at birth.** `mintDefinitionNote` (bootstrap + refit-minted definitions) adds `vocab/<term>` alongside the bare marker; the `:720` comment contract is updated.
...
| `docs/GLOSSARY.md` "vocab definition note" entry (`tags: [vocab]`) | update — definitions carry `tags: [vocab, vocab/<term>]`; family note stays bare-only |
| `docs/architecture/adr.md` ADR-0011 representation-update line (+ its echo near :501) | update — append a dated "Representation update (2026-07-27)" line per the #678 annotation precedent: definitions additionally carry their own `vocab/<term>` self-tag; member semantics unchanged |
| `openspec/specs/vault-vocab-lifecycle/spec.md` | N/A — delta-managed; lands at archive |
| Go comments (`vocab.go:177/222/345`, `vocab_commands.go:720/1068`, `vocab_test.go` comments) | update in code tasks (group 2) |
...
- [Definitions leak into nomination pools once self-tagged] → D2's filter in `buildTagNominations` + a unit test asserting a self-tagged definition is never nominated.
- [Tag edit stales 29 sidecars (content hash covers file bytes)] → backfill writes through the existing write path that refreshes sidecars; test asserts post-backfill embed state is clean (no `StateStale`).
...
2. Run `engram vocab tag-definitions` once against the live vault; verify with `engram vocab stats` (member counts unchanged) and a spot `engram count --group-by tags` diff (each term +1 where its definition exists).
