...
- [x] 1.3 RED→GREEN: skill-note lookup by basename suffix `.skill-<name>.md` on runbook notes carrying `skill_hash` (via existing list/read deps, no new I/O); duplicate matches error naming both
- [x] 1.4 RED→GREEN: offer comparison — per shipped skill: no note → register offer; hash differs → refresh offer; note with no shipped skill → removal offer; hash match or declined hash → nothing. Property test: a run after answering every offer makes no offers
...
- [x] 7.4 Comment on GitHub #757, #758, #759, #760 recording the regression, the pivot, and this change; do not close #760
