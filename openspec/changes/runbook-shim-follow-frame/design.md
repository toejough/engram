...
**D2. The shim has exactly four parts and nothing else.** First action (literal `engram query --phrase … ` before a multi-step task, no skill required); frame for a matched runbook; general behavioral floor; the four re-entry cues. Wording follows the measured rules: name the action, not the purpose (vault note 137); forbid the substitute explicitly; implementation-intention phrasing.
...
- [Over-fire of the first action on trivial tasks] → cue wording keeps the "multi-step task you may have done before" gate; fire-unit is task-init (audited 3.4x, LEDGER `739-audit-fire-unit-over-fire`), not per-turn.
