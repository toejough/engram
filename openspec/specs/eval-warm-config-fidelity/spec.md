# eval-warm-config-fidelity

## Purpose

Ensure that warm trial configs (configs with pre-installed skills and memory) are constructed with verified skill sources and fail fast on configuration errors, eliminating silent failures where skill installations appear successful but actually copied missing or empty source directories.
