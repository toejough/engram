...
The query SHALL emit a Channel 1 payload carrying matched notes and chunks clustered by vector similarity, with per-cluster candidate_l2s nominally ranked from within-cluster members plus tag-nominated cross-cluster notes. Items in Channel 1 appear in `clusters[].members[]`.
...
#### Scenario: Cross-cluster tag nomination
- **WHEN** a note shares a vocab term with the top-3 delivered notes from other clusters
- **THEN** cross-cluster tag nomination is owned by the vault-vocab-lifecycle spec
