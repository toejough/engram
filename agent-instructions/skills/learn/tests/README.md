...
| `baseline-vocab-refit.md` | On `verdict: REFIT_PENDING`, run `engram vocab refit` autonomously; answer any `naming_requests` payload via a fingerprint-echoing `--names` answer file; regenerate on stale-names; on "no structure found" report a plain no-op (no version-bump claim); report loudly. No `--emit-request`, no YAML plan (both removed from the binary). | Step 1.5 |
