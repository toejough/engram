...
2. **Search the union, then make the L2 discoverable.** Run ONE `engram query --synthesis --phrase "<scenario-1>" --phrase "<scenario-2>" …` passing every seed. `--synthesis` unions all the seeds' matches and clusters them **once** (K=0 → a single cluster; no minimum-size floor), so a standard that binds *across* seeds surfaces as one cluster instead of scattering across per-phrase results. The new L2 must appear in the union's `items`; if it does not, the L2 is framed wrong — revise its `situation` toward the scenario and re-embed (`engram embed apply --stale`), then re-check. Discoverability is the whole point of L3; an L2 that only surfaces for its own keywords is invisible exactly when it matters.
3. **Update or create — by SEMANTIC match, not content-compare.** Each union cluster carries `nearest_l3: {path, cosine}` — the closest existing L3 ADR by centroid cosine. For each cluster: if `cosine ≥ 0.9` → **update** that L3: regenerate it folding in the cluster's new L2(s), **preferring the recent L2 where it diverges** from older members. Otherwise → **create** a new L3 from that cluster. Leave every other L3 untouched. (Cluster-once means each new L2 lands in exactly **one** union cluster, so it feeds exactly one update-or-create decision — not several.)
