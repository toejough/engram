...
| You inline-synthesized a cluster *while a dispatch tool was available*       | When a dispatch tool exists, dispatch one subagent per UPDATE/CREATE-band cluster meeting the size precondition. UNINFORMED inline-synthesis from the rep alone is forbidden. (Different from the no-dispatch case: with no dispatch tool you DO run inline — but only after reading every member, which makes it informed.) |
