...
> Step 3a is a **role-split** between two agents — the parent /recall agent reading `nearest_l2.cosine` and choosing a band, and the dispatched synthesis subagent reading every member from disk and writing the L2. When a dispatch tool is available, the parent does not read all cluster members; it sees only the representative (already in `items[]`) and the cluster's `nearest_l2 {path, cosine}`. **Exception — no dispatch tool available:** the parent itself reads the members and runs the L2 write INLINE (see the no-dispatch branch in the parent section below); the read-only-the-rep economy is a dispatch-time optimization, not a license to skip a blocking write. **The band is the parent's call** — it has the cosine in the payload and the subagent does not (the subagent has no centroid, so it cannot recompute the band). The subagent (or the parent inline) *executes the band's assigned action* (fold-in vs new L2), reading members for synthesis quality and the recency tiebreaker — it does NOT re-decide whether to write. The parent does NOT apply the fact-vs-feedback split — that governs the /learn invocation, not the recall reply.
...
2. **No minimum cluster size.** EVERY matched cluster — any size, including size 1 and size 2 — goes through the bands. The cluster matched a real recall, and demand is the relevance proof (spec §2/§5: "no member-exclusion and **no minimum cluster size**"). There is no `size ≥ 3` precondition; do not skip a cluster for being thin. (The old L3 fire-and-forget gate had a size floor; it does **not** apply to lazy L2.)
3. **Band on `nearest_l2.cosine`** (defaults `0.95` / `0.80` — **the harness may override them by naming different values in the recall instruction; use those if given**). **Absent `nearest_l2` ⇒ CREATE:** if a cluster carries **no `nearest_l2` field** (the payload omits it because the vault has no existing L2, or none matched), there is definitionally **no covering L2** — treat it as the **CREATE** band (this is the bootstrap path that crystallizes the first L2s from L1 clusters). Otherwise band on the cosine:
...
4. **BLOCKING — do the writes, then apply them. This is the key behavior.** Unlike fire-and-forget synthesis (which served *future* recalls), these L2 writes are for **your current task**, so they happen before you proceed — *either* by dispatch-and-wait *or* inline. **When a dispatch tool is available:** dispatch the per-cluster UPDATE/CREATE subagents (one per banded cluster, in a single parallel tool-use block), **WAIT for every one to finish**, then **read the resulting notes** (`engram show <basename>`, or the path the subagent reports) and **apply them to your current task** before proceeding. Do not move on while writes are in flight; do not fire-and-forget. **When no dispatch tool is available:** run each band's `engram learn` write **INLINE yourself and apply it** before proceeding (see the no-dispatch branch below) — do not skip. (The NO-OP band writes nothing, so there is nothing to do there.)

**Dispatch shape.** One subagent per cluster in the UPDATE or CREATE band (every such cluster, any size). Subagents are independent; dispatch them in a single parallel tool-use block, then block on the whole batch.

**No dispatch tool? Run the blocking L2 writes INLINE — never skip them.** Verify empirically — try to invoke the dispatch tool; if it is absent or refuses, you do **not** get to skip these writes. They are blocking; the current task needs them. Instead the parent /recall agent performs the dispatched subagent's job itself: for each UPDATE/CREATE-band cluster (any size), **read all the cluster's member notes from disk yourself**, apply the recency-bias tiebreaker on divergence, and run the band's **exact `engram learn` invocation INLINE** (UPDATE: `--target <id> --position continuation`, no `--tier`; CREATE: `--position top --relation` per member, no `--tier` — the commands above) — then read the result back and apply it to your current task. Perform the same steps the [dispatched synthesis subagent](#dispatched-synthesis-subagent-read-members-synthesize-recency-bias) would: read members, execute the band's action, recency-bias on conflict. This relaxes the parent-reads-only-the-rep economy on purpose: that economy exists to save context when *dispatching*; a blocking write your current task needs is worth the member reads.

**This inline path is INFORMED, not the forbidden uninformed inline-synthesis.** The "do NOT inline-synthesize" prohibition (below, and in the red-flag table) targets making an UNINFORMED judgement from the representative alone — synthesizing without reading the members. The no-dispatch fallback reads every member first, so it is informed and is exactly what the skill requires here.

**The "note members as context and skip" carve-out is for fire-and-forget L3 ONLY.** It NEVER licenses skipping the blocking L2 three-band writes. The L2 writes happen either way — dispatch-and-wait when a dispatch tool is available, or inline-and-apply when it is not. They are skipped only when the band is NO-OP (`nearest_l2.cosine ≥ 0.95`). **"User is waiting", "clusters look organized enough", and "synthesis adds latency" are not exits** — they are exactly the moments the L2 your task needs never gets written.
...
- **Apply the freshly-minted L2s.** If you ran UPDATE/CREATE in 3a — whether by dispatched subagents (waited on) or inline (no dispatch tool) — you have the resulting notes in hand (Step 3a is blocking). You may briefly state which conventions you crystallized (or folded into an existing L2) and that you are **applying them to the current task** — that is load-bearing, not scaffolding. Do not narrate the mechanics ("I spawned a subagent for cluster N"), do not paste `[[wikilinks]]`, and do not dump the raw note YAML — paraphrase the convention you are now applying.
...
| You fired-and-forgot the L2 writes instead of waiting | The L2 synthesis is **BLOCKING** — unlike fire-and-forget synthesis for future recalls, these L2s serve your *current* task. Dispatch the UPDATE/CREATE subagents and **wait** (or, with no dispatch tool, run them inline yourself), read the resulting notes, and apply them before proceeding. |
...
| You skipped a small cluster (size 1 or 2), or skipped a cluster with no `nearest_l2` ("nothing to band on") | There is **no minimum cluster size** — band every matched cluster regardless of size. An **absent `nearest_l2` means no covering L2 exists ⇒ CREATE** a new L2 from the cluster. Skipping either is the bootstrap-failure mode (the vault never crystallizes its first L2s). |
| You inline-synthesized a cluster *while a dispatch tool was available*       | When a dispatch tool exists, dispatch one subagent per UPDATE/CREATE-band cluster (any size). UNINFORMED inline-synthesis from the rep alone is forbidden. (Different from the no-dispatch case: with no dispatch tool you DO run inline — but only after reading every member, which makes it informed.) |
| No dispatch tool, so you skipped the blocking L2 writes (noted members "as context" and moved on) | Read the members yourself and run `engram learn` **INLINE** (UPDATE `--target <id> --position continuation`; CREATE `--position top --relation`; no `--tier`), then apply the result to your current task. The skip carve-out is for fire-and-forget L3 ONLY — never for blocking L2 writes. A missing dispatch tool switches you to inline; it does not excuse you from doing the writes. |
| You skipped Step 3a because clusters "look organized" or the user "only asked for recall" | The clusters being organized is a precondition for the gate, not an exit from it. Run the band rule on every cluster. A missing dispatch tool is NOT an exit either — it switches you to the inline path (read members, write `engram learn` inline); verify empirically. |
...
| You read all cluster members from the parent /recall agent *while a dispatch tool was available* | When a dispatch tool exists, the parent reads only the representative and member reads happen inside the dispatched subagent — dispatch instead of opening member files yourself. (No dispatch tool? Reading members in the parent IS the correct inline path — read them, then write `engram learn` inline.) |
