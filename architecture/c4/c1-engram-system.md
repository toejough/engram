...
|---|---|---|---|---|
| <a id="s1-developer"></a>S1 | Developer | Person | Developer who triggers `/prepare`, `/recall`, `/learn`, `/migrate` and authors the work that produces memories | Human, at a Claude Code session |
...
| <a id="s5-anthropic-api"></a>S5 | Anthropic API | External system | LLM service used by the recall pipeline for Haiku ranking and extraction; also the classification step in `/learn` quality gates | `api.anthropic.com` |
