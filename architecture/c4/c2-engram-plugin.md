...
| <a id="s2-n1-skills"></a>S2-N1 | Skills | The system in scope | Markdown skill files (`skills/{prepare,learn,recall,migrate,c4}/SKILL.md`) that Claude Code loads on command or auto-trigger; bodies instruct the agent to call `engram` subcommands and present results | This repo, under `skills/` |
...
|---|---|---|---|---|
| <a id="r1-s1-s3"></a>R1 | S1 | S3 | invokes /prepare, /learn, /recall, /migrate | Claude Code CLI / TTY |
...
| <a id="r10-s2-n3-s6"></a>R10 | S2-N3 | S6 | reads existing feedback + fact TOML during recall/list/show; writes new TOML during learn/update | Local file I/O, TOML |
