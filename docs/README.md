...
| see what's shipped | [`../openspec/specs/`](../openspec/specs/) (behavior specs, primary); [FEATURES.md](FEATURES.md) (pointer + mission rollup) |
...
**`design/`** is the workspace for in-flight, undecided design work only (a `research/` sibling holds landscape research under the same rule when research is in flight). The rule: a doc's behavioral conclusions graduate into `openspec/specs/`, why into ADR, and status into ROADMAP; the file is deleted the same cycle it resolves — steady-state near-empty. A doc found here is either mid-flight or overdue for extraction.
