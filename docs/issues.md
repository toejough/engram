...
Skills generated by `skill_gen.go` don't conform to the criteria defined by the `/skill-creation` plugin (Anthropic best practices) or the `/superpowers` plugin's `writing-skills` skill. This means generated skills may not trigger correctly in Claude's skill selection, may confuse Claude when loaded, and may not integrate well with the broader plugin ecosystem.

### Gap Analysis

**Frontmatter issues:**

| Field | Required | projctl Produces | Gap |
|-------|----------|-----------------|-----|
| `description` format | "Use when..." triggering conditions, max 1024 chars | First 1500 chars of content body | **Critical** — content dump instead of trigger description; exceeds max length |
| `description` person | Third person only | Uncontrolled (whatever content contains) | **Medium** — may use wrong person |
| `name` style | Gerund form, verb-first (`handling-errors`) | `mem:{slug}` from theme (`mem:error-handling`) | **Low** — functional but doesn't match convention |
| Extra fields | Only `name` and `description` | `model`, `user-invocable`, `generated`, `confidence`, `source` | **Low** — non-standard but likely ignored |

**Content structure issues:**

| Criterion | Required | projctl Produces | Gap |
|-----------|----------|-----------------|-----|
| Body structure | Overview → When to Use → Quick Reference → Common Mistakes | Template fallback: flat "Related Memories" list | **High** — template fallback produces wrong structure |
| Body length | Under 500 lines | No enforcement | **Medium** — no guard |
| Concrete examples | Required | Not generated | **Medium** — missing |

**The critical gap** is the `description` field. The superpowers plugin uses descriptions to decide *when* to load skills. The `writing-skills` docs explicitly warn: "Descriptions that summarize workflow cause Claude to follow the description instead of reading the skill body." Currently, projctl dumps content into the description, which is the exact anti-pattern.

### Root Cause

The description field is populated at `skill_gen.go` line ~430 by truncating `skill.Content` (or `skill.Description`, which is set from content). There's no separate step to generate a proper "Use when..." trigger description. The LLM compiler (`CompileSkill`) may produce better content structure, but neither mode generates a trigger-focused description as a distinct output.

### Proposed Fix

1. **Description generation**: Add a separate step (or LLM prompt) that produces a "Use when..." trigger description from the skill's theme and content. Cap at 1024 chars. Must be third person and focus on triggering conditions, not content summary.

2. **Template fallback structure**: Change the non-LLM template to match expected format: Overview → When to Use → Quick Reference → Common Mistakes.

3. **LLM compiler prompt**: Update `CompileSkill` to produce both a trigger description AND structured content body as separate outputs.

4. **Validation**: Add a post-generation check that verifies description starts with "Use when", is under 1024 chars, and body is under 500 lines.

### Files

- `internal/memory/skill_gen.go` — `writeSkillFile()`, description truncation, template fallback
- `internal/memory/optimize.go` — `optimizeCompileSkills()` pipeline
- Whatever implements the `SkillCompiler` interface — LLM prompt for `CompileSkill`

Area: Memory, Skills

### ISSUE-235: CLAUDE.md management doesn't match claude-md-management plugin criteria
...
projctl's CLAUDE.md assessment and update code treats CLAUDE.md as a flat append-only learning log (`## Promoted Learnings` with bullet entries), while the `claude-md-management` plugin defines CLAUDE.md as a structured document with typed sections and a 6-dimension quality rubric. This mismatch means projctl-managed CLAUDE.md files would score poorly under the plugin's quality assessment, and the content projctl promotes lacks the organization the plugin expects.

### Gap Analysis

**Structural mismatch — projctl only uses "Promoted Learnings":**

The plugin expects CLAUDE.md to contain typed sections (Commands, Architecture, Key Files, Code Style, Environment, Testing, Gotchas, Workflow). projctl's `appendToClaudeMD()` always writes to `## Promoted Learnings` regardless of content type. A learning about test commands goes into the same flat list as a gotcha about import ordering.

**No quality scoring:**

The plugin defines a 6-dimension rubric:

| Criterion | Weight | projctl Equivalent | Gap |
|-----------|--------|-------------------|-----|
| Commands/Workflows | 20pts | None | **Missing** — no detection of command presence |
| Architecture Clarity | 20pts | None | **Missing** — no architecture section awareness |
| Non-Obvious Patterns | 15pts | Partially via promoted learnings | **Unstructured** — gotchas not categorized |
| Conciseness | 15pts | Dedup (>0.9 similarity) | **Partial** — removes redundancy but doesn't check filler |
| Currency | 15pts | `isStaleEntry()` (>90 day timestamp) | **Weak** — checks age, not whether content is still accurate |
| Actionability | 15pts | None | **Missing** — no verification commands work |
...
| Command | Description |
|---------|-------------|
| `go test -tags sqlite_fts5` | Run tests with FTS5 support |
```

projctl produces:
```markdown
## Promoted Learnings
- Use `go test -tags sqlite_fts5` for tests (NOT mage)
```

**No section routing:**

When promoting a learning about testing, projctl doesn't route it to `## Testing`. When promoting a gotcha, it doesn't create `## Gotchas`. The `appendToClaudeMD()` function only knows about `## Promoted Learnings`.

**Currency checks are timestamp-only:**

The plugin checks whether commands still work, file paths still exist, and architecture descriptions match reality. projctl's `isStaleEntry()` only checks whether the entry is older than 90 days by timestamp — an entry could be 1 day old and already wrong.

### What Already Aligns

- **Redundancy detection**: `ScanClaudeMD()` finds similar entries (partial conciseness)
- **Narrowness detection**: `isNarrowByKeywords()` catches project-specific content that doesn't belong
- **Deduplication**: `optimizeClaudeMDDedup()` removes >0.9 similarity entries
- **Promote/demote lifecycle**: Content flows between tiers based on utility

### Proposed Fix

1. **Section-aware CLAUDE.md parser**: Extend `ParseCLAUDEMD()` to understand the plugin's expected sections (Commands, Architecture, Testing, Gotchas, etc.), not just `## ` header splitting.

2. **Content routing on promotion**: When promoting a learning, classify it (command, gotcha, architecture, testing pattern, code style) and route to the appropriate section. Create sections as needed.

3. **Quality scoring**: Implement the 6-dimension rubric as a `ScoreClaudeMD()` function that returns per-criterion scores. Use during `optimize --review` to surface quality gaps.

4. **Currency verification**: Check that file paths in CLAUDE.md entries still exist, and that commands are still valid (at minimum, check `package.json` scripts or `Makefile` targets).

5. **Content formatting**: When routing to typed sections, format entries to match the plugin's expected style (tables for commands, tree diagrams for architecture, bullets for gotchas).

### Files

- `internal/memory/claudemd_maintenance.go` — `ScanClaudeMD()`, `ApplyClaudeMDProposal()`
- `internal/memory/consolidate.go` — `ParseCLAUDEMD()`
- `internal/memory/memory.go` — `appendToClaudeMD()`, `RemoveFromClaudeMD()`
- `internal/memory/optimize.go` — `optimizeClaudeMDDedup()`, `optimizeDemoteClaudeMD()`
