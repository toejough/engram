...
| 2 | **#644** OpenCode SQLite session ingest | Corr(reach) / mover / Med | Extends recall coverage to OpenCode history (currently invisible); needs restoring a removed backend | none |
| 3 | **#670** route rubric-refit (fold accrued tier evidence into cold-start priors) | Corr / mover / Med | Unblocked — #674 already shipped the evidence notes; defining the evidence-threshold trigger is the first STEP of the work, not an external blocker | none |
| 4 | **#672** route price table + non-CC cost source | Cost / mover / Low-Med | Real $ in the route mini-report; CC capture already done | none |
| 5 | **Batchable hardening** (one writing pass): **#683** vocab byte-oracle · **#688** recall_time.py hardening · **#679** route dup-aggregate guard · **#680** route copyedit bundle | Dev / hygiene / Low | Real but low-value; each "not a live gap." Batched to amortize cycle overhead; explicitly NOT floated above the higher-value movers despite being axis-1 (value-crossover: Low value sinks them below the Cost item #672) | none |
| 6 | **#695** validate under-load recall-firing fix (placebo control · real-vault retrieval precision · attention-displacement) | Meas / enabler / High | Three caveats the adversarial verifier flagged on the engagement-led `recall.md` cue deployed 2026-07-15 (`LEDGER.md#underload-firing-wording-fix`): isolate engagement-framing from any leading-cue, validate fire→surface precision beyond 5-note fixtures, and rule out the cue displacing the other three decision-moment cues; gates the Parked → Decision-moment recall hook revive trigger. Also carries #686's folded goal — whether the task-kind cue actually fires and doesn't displace the existing decision-moment cues | none |
| 7 | **#648** tune recency constants | Meas / mover / Low-Med | Unblocked — #646 closed 2026-07-19. **Scope shifted by #646's finding:** the recent-fill CHANNEL is redundant for self-captured needs, so #648's lever is the RE-RANK bias (the proven C4i continuity mechanism — recent-relevant outranks old-relevant), i.e. tune the recency half-life / tail-weight, NOT the recent-fill count. Low-Med until a concrete drowning/staleness gauge motivates a sweep. **Scope narrowed again (2026-07-24):** the issue's other named constant, `activationCosineCutoff`, was deleted in commit `efaa2fcd` (activation is now agent-driven via `engram activate`) — the half-life/tail-weight lever above is what remains | none |

### NEXT — blocked by exactly one NOW item
| Item | Axis / type / value | Why here | Blocked on |
|---|---|---|---|
| *(none — #648 moved to NOW after #646 closed 2026-07-19; see NOW rank 7)* | — | — | — |
...
| **#685 Change #2** task-kind glance cue (`agent-instructions/guidance/recall.md`) | Corr | #695's attention-displacement finding (does the shipped engagement-led cue already generalize to task-kind moments, or does a dedicated cue risk displacing the other three decision-moment cues?). Also the fold target for #686 — its doc-staleness goal, reframed from a detector (out of purview) to this recall cue (see Provenance) |
...
| **#637** `--field` query flags | Corr | large surface, no forcing function. **Scope narrowed (2026-07-24):** the pain the body cites — callers jamming subject/behavior/time into one query string — was answered by #639's repeatable `--phrase` flag, now the production recall path, which leaves only true per-field targeting unbuilt and removes the last candidate forcing function | a concrete forcing function |
...
| **#696** pi session ingest · pi harness update-support | Shipped | **#696** (Pi session-file ingest) — shipped 2026-07-24: `engram ingest --auto` sweeps ancestor `.pi` dirs by default, and explicit `--pi-sessions <dir>` roots are accepted, so Pi conversations flow through the same transcript pipeline (JSONL) and auto-index. Removed from NOW rank 3 on 2026-07-24; its sibling **#644** (OpenCode SQLite ingest) remains open in NOW.<br>**pi harness update-support** — shipped 2026-07-24 (pi-agents-integration branch): `engram update` detects `~/.pi` and installs skills to `~/.pi/agent/skills/`; `--with-guidance` deploys the guidance docs to `~/.pi/agent/guidance/` for `~/.pi/agent/AGENTS.md` `@import` (same one-time opt-in semantics as Claude Code); guidance-import detection is per-harness, derived from each `HarnessSpec`. | decided — none |
| #685 (Change #1) | Shipped (staged, not deployed) | **Plan-write doc-surface enumeration grep + Gate A independent-pass clause** — Step 3 now mandates a concept-variant grep + pasted per-file disposition list when a plan alters a repeated invariant; Gate A's docs/diagrams-alignment reviewer independently verifies that list rather than deferring to it. Validated via a new headless probe harness (`please_step3_probe`): a clean single-task probe could NOT reproduce #685's failure (6/6 full enumeration, un-pressured and under pressure — salience-under-load, not a capability gap); a loaded buried-subtask repro DID reproduce it and validated the fix — RED (old text) 5/5 UNDER-enumerated (consistently dropped one off-radar doc) → GREEN (Edit A) 5/5 fully enumerated, +100pp, Fisher exact p≈0.008 (`LEDGER.md#685-doc-enumeration-grep`). Staged to `agent-instructions/skills/please/SKILL.md`; `engram update` deploy and commit held for Joe. Change #2 (task-kind glance cue) deferred — see GATED band → #685 Change #2. | decided — none |
...
| **#658** unbundle recall's $ from build_cost | Shipped | separately-metered recall call landed 2026-06-26 (`fb9bff24` `include_recall`/`recall_only_prompt`, `d99f2fbd` `split_costs`, `8425c9d0` billed `recall_cost`); live at `dev/eval/cumulative/harness.py:134,177,661`, where the docstring states "build_cost is the sum of build rounds and never includes recall". Removed from NOW rank 2 on 2026-07-24. The warm-vs-cold $-premium question it was to enable is NOT answered by the split, and #642 redirected that measurement off this self-seeding harness — it needs a fresh issue framed on real long-session work. | decided — none |
