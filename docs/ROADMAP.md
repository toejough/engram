...
|---|---|---|---|---|
| 1 | **#685** plan-write doc-surface enumeration + task-kind glance cue | Dev / mover / High | Measured recurring gap across 4+ cycles (gate reviewers keep catching doc-scrub the author missed) | none |
| 2 | **#686** old-text echo detector (staleness at the creating commit) | Dev / mover / High | Retro-verified 3/3 against real staleness incidents; compounding | none |
| 3 | **#642** eval harness: self-seeding cold-vs-warm | Meas / enabler(foundation) / High | The value-proof spine — proves whether memory helps at all; front of the eval-value chain (blocker #643 closed). **Axis-2 but crosses above #687 (Dev) on value: it's the highest-leverage mover overall** | none |
| 4 | **#687** run-level self-evaluation (surprise-mining retrospective) | Dev / mover / Med | Generates follow-up issues mechanically; meta-leverage | none |
| 5 | **#658** unbundle recall's $ from build_cost | Meas / enabler / Med | Separable recall_cost unblocks the warm-vs-cold $ question + L4-at-n | none |
| 6 | **#644** OpenCode SQLite session ingest | Corr(reach) / mover / Med | Extends recall coverage to OpenCode history (currently invisible); needs restoring a removed backend | none |
...
| **harder-builds eval** (measurement candidate; designed, unrun; no issue #) | Meas / mover / Med | Joe-specified cross-repo hard-build regime where memory's multi-round payback should show; complements #646 but is a distinct regime | #642 (the cold-vs-warm harness) |
...
| Harder-builds eval baseline (see NEXT band above) | Measured | the easy-build regime measured memory as a net tax (`LEDGER.md#c1-c2-warm-op-negatives`); the regime where multi-round convergence should dominate the tax was designed but never run (Joe specified a cross-repo corpus — spaced-repetition, file-sync, spec-review histories — 2026-06-25). Open candidate: 2–3 hard multi-round builds, warm vs cold, metered — placed in NEXT band above. | the harder-builds eval result itself (once run) |
