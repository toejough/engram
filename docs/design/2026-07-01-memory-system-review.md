...
| 6 | **Edge-payoff probe for the link graph** (does a persisted 2.6 edge ever save a recall cosine misses?) | §4.5 unanticipated-query; note 68 | **⛔ RESOLVED 2026-07-02** by the link-value exploration (`2026-07-02-link-value-exploration.md`): PPR/one-hop traversal KILLED (drops baseline notes; 32–36 collateral regressions); the WINNER is controlled-vocab tag NOMINATION (L6×TAG: 54% recovery of verified misses, delivery +17.3pp overall / +50pp on bridges, above 2σ, zero collateral) | done — see the results doc | resolved | superseded — the "cheapest first step" ran as a full designed exploration per Joe's 2026-07-02 ask |
...
community summaries (static-corpus design, heavy ingest; HippoRAG-style PPR was subsequently
MEASURED and killed — see #6, resolved 2026-07-02); MemoryBank decay (recency already shipped; scale problem we don't
