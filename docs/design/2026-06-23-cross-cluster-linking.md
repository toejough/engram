...
- **Graph-expanded retrieval** (traverse wikilinks to surface bridge notes cosine missed) was a
  *different* deferral here (it fixes *retrieval misses*; the cake's bridge is already retrieved). It
  was **built then reverted as slice 2** (2026-06-23): the binary-level proof worked (a single narrow
  query misses the bridge; expansion surfaces it), but the **real-skill A/B across 3 fixtures showed 0
  marginal value** — recall issues 10 broad phrases and reaches the bridge via cosine without any hop.
  Retrieval is not the bottleneck; the multi-phrase recall is itself query expansion. See the
  EXPERIMENT-LOG 2026-06-23 slice-2 entry.
