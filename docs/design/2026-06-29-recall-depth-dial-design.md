...
read-vs-write split**: a cheap `glance` (retrieve, recency-resolve, apply — no vault writes) vs the full `deep`
...
(the O2/L2/L3a *binary* safe-cuts) as its enabler. This ask **revives §P7 and generalizes** it — and
...
- **`consider` (middle rung):** *data-gated* — start at 2 rungs; **Item 1 decides** whether a 3rd rung (or a
  finer/continuous grain) is warranted. We don't assert "a slider is over-engineering" up front — we let the
  measurement say.
...
1. **The cost is in the skill procedure, not the binary.** Payload `--n-X` flags barely move it (notes
   77/79/100: −61% payload ≈ nothing end-to-end) and mostly already exist. The dial is primarily a
   **recall-skill restructure** (phrase-count + crystallize-or-not), with the binary contributing #657's safe
   cuts (O2 inline candidate content; L2 skip-2.5C-when-empty).
...
recall-skill mode + land #657's safe binary cuts (O2, L2). Ship `glance`-as-default **only for the
moment-classes Item 1 validated**; others keep `deep`. **Gate** with the C3/C4i/C5/C6 trap harness (note 102)
