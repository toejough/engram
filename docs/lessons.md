...
**23. Specifications co-evolve bidirectionally — build upward propagation into the process, not as checkpoints.**
Lower layers discover constraints that upper layers didn't account for (impossibilities, over-constraints, under-specifications). This isn't exceptional — it's the normal signal path. A one-time feasibility check at a single boundary is an ad-hoc band-aid; the real need is a uniform mechanism at every layer boundary. Bidirectional dirty flags (downward for derivation staleness, upward for constraint discovery) with absorption-first escalation handle this naturally. Each layer absorbs constraints locally before escalating; most constraints resolve one layer up. Only fundamental impossibilities cascade. *(Surfaced when: evaluating Twin Peaks model against the depth-first tree traversal — the 30-minute DES→ARCH feasibility check created an awkward extra step that didn't generalize.)*

**24. Prefer transparent file storage over databases when the access patterns allow it.**
