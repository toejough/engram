...
flowchart TB
    ...
    subgraph PP[engram prune — process]
      prune["K12 · prune (operator-run GC)<br/>RunPrune · read manifest · drop stale per-source manifest entries (index files preserved); --empty removes 0-byte .jsonl index files (#694); --duplicates removes non-canonical duplicate index files, gated on record-subset coverage (ADR-0021)"]
    end
    subgraph PU[engram update — process]
      upd["K9 · update<br/>go install, re-exec fresh binary for sync (ENGRAM_UPDATE_REEXEC, ADR-0023); sync artifacts to engram-owned roots, materialize symlinks; dark migration, cleanup; --with-guidance; manifest fallback (ADR-0022)"]
    end
    ...
    skills -->|"shell engram query --phrase / --text"| query
...
sequenceDiagram
    ...
    Note over L,V: at later, SEPARATE `engram query` (K6) processes (see the query flow above) — A-note competes in the main matched set like any fact/feedback note, Q-note EXCLUDED at all four query-pipeline seam points (isQueryExcludedKind) — applyFloorAndCap, isFloorQualifyingNote (capWithNoteFloor reservation), mergePhraseIntoUnion (per-phrase union), explore-half TermIndex build (loadAllVaultNotesMeta in query_vault_meta.go, guarded by isQueryExcludedKind before a note's terms populate TermIndex)
