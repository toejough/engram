...
flowchart LR
    ...
    engram -->|"R3: reads & writes notes, MOCs, MEMORY.md"| vault
...
| <a id="s4-agent-memory-vault"></a>S4 | Agent-memory vault | External system | Luhmann zettelkasten on the local filesystem — `Permanent/`, `MOCs/`, `MEMORY.md` | `$ENGRAM_VAULT_PATH` or `$XDG_DATA_HOME/engram/vault` (typically `~/.local/share/engram/vault`) |
...
| <a id="r3"></a>R3 | S2 Engram | S4 Agent-memory vault | Reads & writes notes, MOCs, and `MEMORY.md` under a `flock`-held vault lock; rendered as a single unidirectional arrow per the C4 read+write CRUD convention |
