...
| <a id="s2-engram"></a>S2 | Engram | System in scope | Persistent memory for LLM coding agents: reads & writes a Luhmann zettelkasten vault, reads per-harness session transcripts via markers, self-updates, and provides `engram prune` (operator-run GC: detaches chunk-index manifest entries whose source file no longer exists, preserving the embedded chunks on disk — outside the recall/learn/please/update flows) | This repo (`cmd/engram/`, `internal/`, `skills/`) |
