...
| <a id="s3-llm-coding-harness"></a>S3 | LLM coding harness | External system | Hosts engram's slash commands and subprocess-invokes the engram CLI. Engram skills (recall, learn, write-memory) are loaded by the harness's skill mechanism; the please, route, and curate procedures are vault runbooks fetched through `engram query`, not loaded skills. | Claude Code (`~/.claude/`), Pi (`~/.pi/agent/`) |
