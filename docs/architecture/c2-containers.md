...
flowchart TB
    ...
    cli -->|"engram update: go install, then re-execs the fresh binary for the sync phase (ENGRAM_UPDATE_REEXEC loop guard, ADR-0023), which syncs agent-instructions/{skills,commands,guidance} to engram-owned roots and materializes symlinks; removals propagate; dark migration on first sync (ADR-0022)"| gotool
