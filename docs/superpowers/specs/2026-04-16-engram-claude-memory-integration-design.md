...
**Scope:** Make engram complementary to (not competitive with) Claude Code's built-in memory by cross-searching external sources during recall/prepare. Write path (learn/remember) is unchanged — engram-internal dedup only.
...
- Preserve engram's role as the place for **task-triggered, situation-framed, cross-session memories**. External sources own different content types (prescriptive rules, curated notes, packaged workflows). When the same lesson appears in both, engram's task-triggered surfacing is the value-add.

## Non-goals

- **Cross-source dedup on write.** `learn` / `remember` do not look at external sources. If a lesson exists in CLAUDE.md / auto memory / skills but the agent still failed to apply it here, engram saving the same lesson is *desirable reinforcement* — task-triggered surfacing complements the existing source.
...
A new subsystem, **external sources**, lives at `internal/externalsources/`. It owns discovery, file reading, and the per-invocation file cache. The existing recall pipeline in `internal/recall/` gains new phase types that consume from external sources. The write path (`learn` / `remember`) is **untouched** — engram-internal dedup proceeds exactly as today.

### Cross-search pipeline (six phases, recall/prepare only)

The existing three-phase recall pipeline becomes six phases. Each phase Haiku-extracts relevant content into a shared 10 KB buffer. Phases run in priority order; once the buffer fills, downstream phases are skipped.
...
| Rank step returns malformed list                 | Treat as empty rank; phase contributes nothing; pipeline continues.     |
| Haiku API error within a phase                   | Skip that phase, log warn, continue. Other phases unaffected.           |
| File grows mid-invocation between rank + extract | Use cached content (locked at first read); next invocation gets fresh.  |

---

## Write path (learn / remember)

**Unchanged from today.** External sources are not consulted on save:

- Engram-internal dedup runs as it does now (DUPLICATE returned for similar existing engram memories; resolution via `engram update` to broaden situation).
- No external dedup. No `overlaps_with` metadata. No `--force` flag added in this spec.

Rationale: when a lesson already exists in CLAUDE.md / auto memory / a skill but the agent still failed to apply it in the moment, engram saving the same lesson is *desirable reinforcement* — task-triggered surfacing complements the existing source. Cross-source dedup on save would suppress that reinforcement and waste cycles on every save.
...
No new tests required — `learn` / `remember` behavior is unchanged. Existing engram dedup tests cover the write path.
...
- Concrete prompt templates for each rank Haiku call.
