...
- [ ] **Step 3b:** `git mv` the three specs to their Keep-list targets — the two architecture-bound catalogs drop their date prefixes (`memory-invariants.md`, `memory-system-rigor.md`, matching the report §1 tree and every already-committed citer); the subprocess spec keeps its dated basename in docs/design/.
