...
```
  ...
  Mechanical rename at the remaining non-test usages — 5 identifier renames + 2 comment mentions after Step 2 deletes the :996 sniff line (`grep -rnw "typeVocab" --include="*.go" internal/` and follow the compiler) — includes the two `Tags: []string{typeVocab}` mint sites in `vocab_commands.go` and the reads in `isVocabDefinitionNote`, `nonVocabTags`, `vocabTermsFromTags`'s comment.
...
- [ ] **Step 4: Deploy.** Verify cwd first: `git rev-parse --show-toplevel` must print the engram repo root (running `engram update` from elsewhere pulls remote main instead of deploying local edits). Then `engram update`. Verify the deploy landed with WRAP-INSENSITIVE checks (the file wraps at ~100 chars; a plain grep false-fails if a re-wrap splits the fragment — measured: the command shape reproduces 1 on a known-wrapped sentence):
  `tr -s '[:space:]' ' ' < ~/.claude/skills/recall/SKILL.md | grep -c "surfaces tag-sharing notes at query time"` = 1 AND the same pipeline with `"connects the new note to related notes structurally"` = 0 (measured pre-edit baselines: 0 and 1). Failure triage: new-fragment = 0 AND old-fragment = 1 → stale deploy, re-run `engram update` from the repo root; new-fragment = 0 AND old-fragment = 0 → the repo-source edit itself dropped or reworded the fragment — fix `skills/recall/SKILL.md` to contain the Step 2 sentence verbatim, then redeploy.
