...
- No dead Go code was identified outside the 649 surface; `targ check-full` (the repo's gate) flags unused symbols, so a green check post-cleanup is the no-dead-code guarantee. A deeper whole-repo dead-code audit is out of this cleanup's scope.
...
- `internal/cli/targets_test.go` (re-verify NEW-1, KEPT file) — `TestTargets` "returns expected target count" asserts `HaveLen(14)` → change to the post-removal count (**11**), update the accompanying comment, and delete the subtests that invoke `learn episode` / `transcript` / and `TestTargets_MigrateEpisodes`.
- `internal/cli/signal_test.go` (re-verify NEW-2, KEPT file) — the `HaveLen(14)` target-count assertion (≈line 74) → **11**.
